...
7. **Research, one requirement at a time** (only requirements with a criterion in scope). Invoke **`yamlet-code-research`** (`/yamlet-code-research <input>`) with: the code root, the contract, the requirement with its in-scope criteria verbatim (their `reads`/`writes` included), and the obligations of the records deciding it (`ADR-nnnn#R-n` + `must`). It returns, per item, where the behaviour lives (`file:line`), what the code does there, deviations, related tests, the stored state the criteria touch, and which directories it read closely or skimmed. Record the directories: `yamlet techspec analysis TS --deep DIR --skimmed DIR` (lists accumulate).
8. **Verdicts.** Decide `met: true` only when *every* `shall` (or the `must`) is observably satisfied at a cited reference; a partial, a wrong value, or the right place with the wrong behaviour is `met: false` with the references as evidence and a one-line `--note` saying what differs. **Before recording `met: true`**, invoke **`yamlet-evidence-challenger`** (`/yamlet-evidence-challenger <input>`) with the criterion or obligation verbatim (a criterion's `writes` included: each is checked like a `shall`) and the exact references. `REFUTED` → record `met: false` with its reason as the note.
...
9. **State inventory, across the whole plan.** Start from what the specs declare: every field the criteria in scope read or write, plus `yamlet systems DIR --system=S --state` for every other scope of the system touching the same fields — in the plan or not. Add each research report's STATE: where the code declares each field, or `absent`. Working notes, not a file. A field that is absent, or declared in a way a criterion cannot use, is schema work for step 11. Two scopes on one field, at least one writing it, are a **contended pair** for step 10. Stored state a criterion's code path touches that it does not declare (research lists it as UNDECLARED) is a spec gap: route the user to `yamlet-author` (`add-state`) before planning on it.
