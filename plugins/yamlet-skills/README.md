...
| `yamlet-code-research` | Read-only research inside the tech spec flow: per requirement, where each in-scope criterion's behaviour (and each obligation of the records deciding it) lives, what the code does there, deviations from each `shall`, related tests, the stored state the criteria touch and who writes it. Documents, never judges. |
...
| `yamlet-adr-challenger` | Adversarial gate after the dimensions and before any option: is the question answerable, are the forces outside the author's control, does each dimension state a threshold, will the option set be honest. Judgement only; structure is the verifier's. |
