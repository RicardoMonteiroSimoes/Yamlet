...
| `yamlet-code-research` | Read-only research inside the tech spec flow: per requirement, where each in-scope criterion's behaviour (and each obligation of the records deciding it) lives, what the code does there, deviations from each `shall`, related tests. Documents, never judges. |
