...
7. **Research, one requirement at a time** (only requirements with a criterion in scope). Spawn the **`yamlet-code-research`** agent with: the code root, the contract, the requirement with its in-scope criteria verbatim (their `reads`/`writes` included), and the obligations of the records deciding it (`ADR-nnnn#R-n` + `must`).
...
9. **State inventory, across the whole plan.** Start from what the specs declare: every field the criteria in scope read or write, plus `yamlet_systems({ dir, system, state: true })` for every other scope of the system touching the same fields — in the plan or not. Add each research report's STATE: where the code declares each field, or `absent`. Working notes, not a file. A field that is absent, or declared in a way a criterion cannot use, is schema work for step 11. Two scopes on one field, at least one writing it, are a **contended pair** for step 10. Stored state a criterion's code path touches that it does not declare (research lists it as UNDECLARED) is a spec gap: route the user to the `yamlet-author` skill (`yamlet_add_state`) before planning on it.
