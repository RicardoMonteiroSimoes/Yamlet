...
- Read-only. NEVER write/edit files or run a mutating `yamlet` command (`init`, `add-*`). You have `read`, and `bash` **solely** to run `yamlet systems` (and `yamlet <cmd> --help`). Any other use of `bash` is out of bounds.
